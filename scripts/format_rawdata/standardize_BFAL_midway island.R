#---------------------------------------------------------------------------####
## Black-footed Albatross - Midway Island ## 
...
meta <- fread("data/raw_data/midway island/meta.csv") %>% filter(Spp == "BFAL")
meta16 <- fread("data/raw_data/midway island/meta_2015_2016.csv") %>% filter(Species == "BFAL") %>% 
  mutate(bird_id=as.character(BirdNum + 100))
...
rawdata <- rbindlist(
  lapply(seq_along(subfolders), function(x) {
    onef      <- subfolders[x]
    files     <- list.files(onef, pattern="BFAL", full.names = T)
    filenames <- tools::file_path_sans_ext(list.files(onef, pattern="BFAL", full.names = F))
      ...
      lapply(seq_along(files), function(i){
        ...
        if( tools::file_ext(files[i]) == "csv" ){
          ...
          if( str_detect(files, pattern = "all") ){
            one <- dplyr::filter(one, species == "BFAL" & year == 2016) %>% # 2014/15 data dplctd in another folder
...
goodyrs <- BFAL_summ$season_year[BFAL_summ$breed_stage %in% c("chick-rearing", "brood-guard") & BFAL_summ$n_birds > 5]
