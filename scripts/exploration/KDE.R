...
# ssizes <- fread(paste0("data/summaries/sp_site_nyears_Xtracks_", stage, ".csv"))
...
for(i in seq_along(tfiles)){
  ...
  onemeta <- meta[meta$scientific_name %in% sp,]
  ...
  HVALS <- findScale(TD, scaleARS = F, sumTrips=tsumm)
  HVALS

  # choose one (log10 of foraging range)
  h <- HVALS$mag
  h

  meta[meta$scientific_name %in% sp,]$h <- h
  meta[meta$scientific_name %in% sp,]$med_max_dist <- HVALS$med_max_dist
  ...
  filename <- paste0(outfolder, paste(sp, site, bstage, h, sep = "_"), ".rds")
  saveRDS(UD, filename)
  # 
  ...
  # filename <- paste0(outfolder, paste(sp, site, bstage, "map", sep = "_"), ".png")
  ...
  f_summ[[i]] <- data.frame( scientific_name = sp,
...
### Average together individual UDs to create a pooled UD (across years) ###