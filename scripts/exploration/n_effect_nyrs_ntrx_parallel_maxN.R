...
# this version goes to the maximum N in a year (by dropping nyears)
...
## LOOP ! ---------------------------------------------------------------------
udfiles     <- udfiles[c(3, 7, 10)]
udfilenames <- udfilenames[c(3, 7, 10)]
iaudfiles   <- iaudfiles[c(3, 7, 10)]
...
foreach(
  ...
  maxn <- max(yrly_ss$n_ids) # what is minimum single-year sample size?
  
  avg_out_list <- vector("list", length(seq_len(minn)))
  its_out_list <- vector("list", length(seq_len(minn)))
  
  # for(e in seq_len(maxn)){ ## go to maximum yearly sample size
  for(e in seq_len(minn)){   ## go to minimum "" "" ""
    print(paste("sample size:", e))
    
    if(e > minn){
      yr_ss_f <- yrly_ss %>% filter(n_ids >= e)
      years <- unique(yr_ss_f$season_year) # years with data
    ...
    yr1_sample <- sample_y1(its, n_yr, yr_ss_f, field_year = "season_year") 
    ## even weights for all years
    yr1_sample <- sample(yr_ss_f$season_year, its, replace = T)
  ...
  filename <- paste0("data/exploration/test/1", asp, "_", asite, "_", "i", its, ".rds")
  saveRDS(avg_out_df, filename)
  
  # filename <- paste0("data/analysis/n_effects_its/", asp, "_", asite, "_", "i", its, ".rds")
  filename <- paste0("data/exploration/test/2", asp, "_", asite, "_", "i", its, ".rds")
