...
## LOOP ! ---------------------------------------------------------------------
udfiles     <- udfiles[1]
udfilenames <- udfilenames[1]
iaudfiles   <- iaudfiles[1]
...
foreach(
  ...
  ## Make dataframe of all KDE/bird IDs to select from ------------------------
  ...
  for(k in seq(its) ){
    ...
    ## Calculate area of the ref dist -----------------------------------------
